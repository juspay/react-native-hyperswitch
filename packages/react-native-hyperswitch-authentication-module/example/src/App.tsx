import { 
  ...
  generateChallenge,
  threeDSProvider 
} from '@juspay-tech/react-native-hyperswitch-authentication-module';
...
export default function App() {
  ...
  console.log('-- HyperswitchAuthModule is available:', isAvailable);

  const handleInitThreeDs = () => {
    try {
      const configuration = {
          provider: threeDSProvider.cardinal,
          jwtToken: "jwt_token_for_cardinal",
  ...
  const handleGenerateAReqParams = () => {
    try {
      ...
      const directoryServerId = 'test_directory_server_id';
      const cardBrand = 'visa';

      generateAReqParams(
        messageVersion,
        (aReqParams, status) => {
          console.log('AReq Params:', aReqParams);
      ...
      console.error('Failed to generate AReq params:', error);
    }
  };
  ...
  const handleReceiveChallengeParams = () => {
    try {
      ...
      recieveChallengeParamsFromRN(
        ...
        (status) => {
          console.log('Challenge Params Status:', status);
          setChallengeParamsResult(`Status: ${status.status}\nMessage: ${status.message}`);
        }
      );
    } catch (error) {
      setChallengeParamsResult(`Error: ${error}`);
      console.error('Failed to receive challenge params:', error);
    }
  };
  ...
  const handleGenerateChallenge = () => {
    try {
      generateChallenge((status) => {
        console.log('Generate Challenge Status:', status);
